...
shinyServer(function(input, output, session) {
  ...
  output$organism <- renderUI({
    ...
    selectizeInput("organism", "Landrace variety:", choices, selected = "Japonica", multiple = FALSE, options = list(dropdownParent="body"))
  })
  ...
  output$zChart <- renderChart2({
    ...
    if(input$organism ==  "Japonica"){#strand is '+' or '-'
